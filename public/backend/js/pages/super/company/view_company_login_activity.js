...
var company_login_activity = $('#company_login_activity').DataTable( {
   "processing": true,
   "serverSide": true,
   "deferRender": true,
   "ajax":{
    "url": baseURL + "tableResponse/Company/CompanyLoginActivity/ajax_list",
    "type": "POST",
    data: function(d) {
...
function format ( d ) {
    ...
    $('#company_login_activity tbody').on('click', 'td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = company_login_activity.row( tr );        
...
$('#company_login_activity tbody').on( 'click', '#view', function () {
    var data = company_login_activity.row($(this).parents('tr')).data();
